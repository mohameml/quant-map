...
<Hint n={1}>
Start by computing the expected gain $\mathbb{E}[G]$. Since $N \sim \mathrm{Geom}(1/2)$, we have $\mathbb{P}(N = n) = (1/2)^n$ and the gain is $G = 2^N$. Sum the series.
</Hint>

<Hint n={2}>
The expected gain is infinite! To resolve this paradox, consider using a utility function $U(x) = \ln(x)$ instead of raw monetary value. Compute $\mathbb{E}[\ln(G)]$ and find the certainty equivalent.
</Hint>
