...
tags: ["geometric-distribution", "harmonic-numbers", "linearity-of-expectation", "classic"]
---
...
<Hint n={1}>
Break the process into phases: let $T_i$ be the number of purchases needed to find the $i$-th new card, given you already have $i-1$ distinct cards. What is the probability of getting a new card in this phase?
</Hint>

<Hint n={2}>
Each $T_i$ follows a geometric distribution with parameter $p_i = \frac{N - (i-1)}{N}$. Use linearity of expectation on $T = T_1 + T_2 + \cdots + T_N$.
</Hint>
...
Using the classical expansion $H_N = \ln(N) + \gamma + O\!\left(\frac{1}{N}\right)$, where $\gamma \approx 0.5772$ is the Euler–Mascheroni constant:
