...
<Hint n={1}>
	Think about pairs of flips. Among the four possible outcomes
	$HH$, $HT$, $TH$, $TT$, can you find two that have the same probability
	regardless of $p$?
</Hint>

<Hint n={2}>
	Consider rejecting symmetric outcomes ($HH$ and $TT$) and only
	keeping asymmetric ones ($HT$ and $TH$). What are their respective
	probabilities?
</Hint>

<Hint n={3}>
	To rigorously prove fairness, define the first accepted trial
	$N$ and decompose $\mathbb{P}(Y=H)$ as a sum over all possible
	values of $N$. You will obtain a geometric series.
</Hint>
...
Consider an *accepted* trial, that is, the event
