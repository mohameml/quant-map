...
<Hint n={1}>
Use symmetry to classify the 8 vertices into types based on their distance from the target vertex: Type 0 (start, distance 3), Type 1 (distance 2), Type 2 (distance 1), Type 3 (target, absorbing).
</Hint>

<Hint n={2}>
Each vertex of a cube has exactly 3 neighbors. From Type 0, all 3 neighbors are Type 1. From Type 1, one neighbor is Type 0 and two are Type 2. From Type 2, two neighbors are Type 1 and one is Type 3. Set up the absorption time equations.
</Hint>
