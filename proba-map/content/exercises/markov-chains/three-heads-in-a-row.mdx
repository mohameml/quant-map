...
<Hint n={1}>
    Model this with a Markov chain with four states $S_0, S_1, S_2, S_3$ based
    on the current streak of consecutive heads. State $S_3$ is the absorbing
    state.
</Hint>
...
- $S_3$: 3 consecutive heads (absorbing state — goal reached)
