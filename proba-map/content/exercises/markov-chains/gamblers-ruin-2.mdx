...
<Hint n={2}>
    For Q2, find a function $f(x) = \lambda^x$ such that $f(X_k)$ is a
    martingale when $p \neq 1/2$. The condition $p\lambda + q\lambda^{-1} = 1$
    leads to $\lambda = q/p$.
</Hint>

<Hint n={3}>
    For Q3, show that $M_k = X_k^2 - k$ is a martingale by computing $E[X_
    {k + 1}^2 \mid \mathcal{F}_k]$. Apply the Optional Stopping Theorem to this
    new martingale.
</Hint>
