...
<Hint n={1}>
    Model this with a Markov chain with $n+1$ states $S_0, \dots, S_n$ based on
    the current streak of consecutive heads. Write the absorption time equations
    for each state.
</Hint>

<Hint n={2}>
    Define the successive differences $d_k = E_k - E_{k + 1}$. Show that $d_0 =
    \frac{1}
    {p}$ by simplifying the equation for $E_0$.
</Hint>
...
- $S_n$: $n$ consecutive heads (absorbing state — goal reached)
...
_Base case_ ($k = 0$): Already shown: $d_0 = \frac{1}{p}$.

_Inductive step:_ Assume $d_j = \frac{1}{p^{j+1}}$ for all $0 \le j \le k-1$. We show $d_k = \frac{1}{p^{k+1}}$.
