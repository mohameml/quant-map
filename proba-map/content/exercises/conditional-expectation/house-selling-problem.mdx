...
tags: ["optimal-stopping", "bellman-equation", "threshold-strategy", "dynamic-programming"]
---
...
<Hint n={2}>
For $X \sim \mathcal{U}[0,1]$, compute $\mathbb{E}[X \mid X \geq s] = \frac{s+1}{2}$. Substitute into the Bellman equation and solve for $V(s)$.
</Hint>

<Hint n={3}>
Differentiate $V(s)$ with respect to $s$, set $V'(s) = 0$, and solve for the optimal threshold $s^*$. Check the validity condition $s^* \geq 0$.
</Hint>
